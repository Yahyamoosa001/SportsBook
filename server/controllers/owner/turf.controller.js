...
export const turfRegister = async (req, res) => {
  ...
  try {
    ...
    // Extract openTime and closeTime for the new operatingHours structure
    const { openTime, closeTime, ...otherData } = req.body;

    const turf = new Turf({
      image: turfImage.secure_url,
      owner,
      ...otherData,
      // Set both new structure and legacy fields for backward compatibility
      operatingHours: {
        openTime: openTime,
        closeTime: closeTime,
        closedDays: [] // Default to no closed days
      },
    ...
    return res
      .status(201)
      .json({ success: true, message: "Facility created successfully" });
  } catch (err) {
